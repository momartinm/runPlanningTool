...
:effect (and (at start (not (empty ?s))) (at end (full ?s)) (at end (have_soil_analysis ?x ?p)) (at start (not (at_soil_sample ?p)))
		)
)
...
:effect (and (at start (not (empty ?s))) (at end (full ?s)) (at end (have_rock_analysis ?x ?p))  (at start (not (at_rock_sample ?p)))
		)
)
...
(:durative-action calibrate
 ...
 :duration (= ?duration 5)
 :condition (and (at start (equipped_for_imaging ?r)) (at start (calibration_target ?i ?t)) (over all (at ?r ?w)) (at start (visible_from ?t ?w)) (at start (on_board ?i ?r)) (at start (not_calibrated ?i ?r))
		)
 :effect (and
		(at start (not (not_calibrated ?i ?r)))
...
(:durative-action take_image
 ...
 :effect (and (at end (have_image ?r ?o ?m)) (at end (not_calibrated ?i ?r)) (at start (not (calibrated ?i ?r)))
		)
)
