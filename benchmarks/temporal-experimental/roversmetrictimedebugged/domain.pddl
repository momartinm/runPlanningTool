...
(:predicates (at ?x - rover ?y - waypoint) 
	     ...
	     (not_calibrated ?c - camera ?r - rover)
...
:effect (and (at start (not (empty ?s))) (at end (full ?s)) (at start (decrease (energy ?x) 3)) (at end (have_soil_analysis ?x ?p)) (at start (not (at_soil_sample ?p)))
		)
)
...
:effect (and (at start (not (empty ?s))) (at end (full ?s)) (at end (have_rock_analysis ?x ?p)) (at start (decrease (energy ?x) 5)) (at start (not (at_rock_sample ?p)))
		)
)
...
:effect (and (at start (not (full ?y))) (at end (empty ?y))
	)
)
...
(:durative-action calibrate
 ...
 :condition (and (at start (equipped_for_imaging ?r)) (at start (>= (energy ?r) 2)) (at start (calibration_target ?i ?t)) (over all (at ?r ?w)) (at start (visible_from ?t ?w)) (at start (on_board ?i ?r))
		 (at start (not_calibrated ?i ?r))
		)
 :effect (and (at end (calibrated ?i ?r)) (at start (not (not_calibrated ?i ?r)))  (at start (decrease (energy ?r) 2)))
)
...
(:durative-action take_image
 ...
 :effect (and (at end (have_image ?r ?o ?m)) (at start (decrease (energy ?r) 1)) (at end (not_calibrated ?i ?r)) (at start (not (calibrated ?i ?r)))
		)
)
