...
FFEvent::FFEvent(instantiatedOp* a, const double & dMin, const double & dMax)
        : action(a), time_spec(Planner::E_AT_START), minDuration(dMin), maxDuration(dMax), pairWithStep(-1),
        getEffects(true),
...
FFEvent::FFEvent(instantiatedOp* a, const int & pw, const double & dMin, const double & dMax)
        : action(a), time_spec(Planner::E_AT_END), minDuration(dMin), maxDuration(dMax), pairWithStep(pw),
...
FFEvent::FFEvent(instantiatedOp* a, const int & s, const int & pw, const double & dMin, const double & dMax)
        : action(a), time_spec(Planner::E_OVER_ALL), minDuration(dMin), maxDuration(dMax), pairWithStep(pw),
...
FFEvent::FFEvent(const int & t)
        : action(0), time_spec(Planner::E_AT), minDuration(-1.0), maxDuration(-1.0), pairWithStep(-1), getEffects(true),
...
FFEvent::FFEvent()
        : action(0), time_spec(Planner::E_AT_START), minDuration(0.0), maxDuration(0.0), lpTimestamp(-1.0),
...
void FFEvent::printPlanToFile(const list<FFEvent> & toPrint, const std::string fileName)
{
    tms refReturn; 
    times(&refReturn);
    ...
    for (; sortedItr != sortedEnd; ++sortedItr) {
        ...
        for (; iItr != iEnd; ++iItr) {
            ...
            if (planItr->lpTimestamp < 0.0000001) {
                ...
                outputFile << threeDP(planItr->lpTimestamp);
            }
            outputFile << ": " << *(planItr->action) << " ";
            actions++;
            if (planItr->pairWithStep >= 0) {
                const double dur = endTS[planItr->pairWithStep] - planItr->lpTimestamp;
                outputFile << " [" << threeDP(dur) << "]";
                #ifdef STOCHASTICDURATIONS
                outputFile << ";\t\t {" << planItr->stochasticTimestamp->getTimestampForRPGHeuristic() << "} {" << endSTS[planItr->pairWithStep] << "}";
                #endif
            } else if (RPGBuilder::getRPGDEs(planItr->action->getID()).empty()) {
                outputFile << " [" << threeDP(RPGBuilder::getNonTemporalDurationToPrint()[planItr->action->getID()]) << "]";
                #ifdef STOCHASTICDURATIONS
                outputFile << ";\t\t {" << planItr->stochasticTimestamp->getTimestampForRPGHeuristic() << "} {" << EPSILON + planItr->stochasticTimestamp->getTimestampForRPGHeuristic() << "}";
                #endif                        
            } else {
                assert(false);
            }
    ...
    outputFile << ";Actions: " << actions << endl;
    outputFile << ";Planning time: " << wholesecs << endl;
